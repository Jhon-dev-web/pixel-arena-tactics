    ...
    <div className="modal-backdrop">
      <div className="modal dungeon-modal">
        ...
        <div className="dungeon-body">
          <div className="gear-section-title">{gardenText('slotsTitle')}</div>
          <div className="garden-grid">
            {slotStatuses.map((status, i) => {
              ...
              return (
                <div className={`garden-slot${def ? '' : ' empty'}${status.ready ? ' equipped' : ''}`} key={i}>
                  {def ? (
                    <>
                      <span className="garden-slot-name">
                      ...
                      <span className="camp-mine-time">
                        {status.ready ? gardenText('ready') : gardenText('growing')}
                        {!status.ready && ' · ' + gardenText('timeRemaining').replace('{t}', formatRemaining(status.remainingMs))}
                      </span>
                      <div className="repair-btns">
                        <button
                          className={`craft-btn forge repair-action-btn${status.ready ? ' blink' : ''}`}
                          onClick={() => onHarvest(i)}
                        ...
                        <button className="craft-btn blessed repair-action-btn" onClick={() => onCancel(i)} data-ui>
                          {gardenText('cancel')}
                        </button>
                      </div>
                    </>
                  ) : (
                    <span className="garden-slot-empty">{gardenText('emptySlot')}</span>
                  )}
                </div>
              );
            })}
          ...
          <div className="gear-section-title">{gardenText('plantTitle')}</div>
          {PLANTS.map((plant) => {
