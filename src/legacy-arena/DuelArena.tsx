/* Legacy 1v1 duel arena — the game's original main loop before the Camp/Dungeon
   loop replaced it. Archived here, decoupled from src/App.tsx, so it can seed a
   future standalone mode (e.g. a multiplayer arena/minigame). Not rendered by
   the current app — nothing imports this file.

   To revive it: render <DuelArena save={...} onSaveChange={...} onExit={...} />
   from a parent that owns persistence, the way src/App.tsx used to inline it. */
...
export default function DuelArena({
  ...
  const [save, setSaveState] = useState<SaveData>(initialSave);
  const [player, setPlayerState] = useState<FighterState>(() => makePlayer(initialSave));
  const [enemy, setEnemyState] = useState<FighterState>(() =>
    makeEnemy(enemyKindForDuel(initialSave.victories + 1), initialSave.victories + 1),
  );
  const [enemyKind, setEnemyKindState] = useState<EnemyKind>(() => enemyKindForDuel(initialSave.victories + 1));
  ...
  const onSaveChangeRef = useRef(onSaveChange);
  ...
  const doTurn = async (action: PlayerAction) => {
    ...
    const result = resolveTurn(action, playerRef.current, enemyRef.current, saveRef.current, getEnemyDef(enemyKindRef.current));
  ...
  const nextDuel = () => startDuel();
  const retryDuel = () => startDuel();
  ...
  return (
    <div className={`stage${shaking ? ' shaking' : ''}`}>
      ...
      <button className="return-camp" onClick={onExit} data-ui>
        {t('camp.returnCamp')}
      </button>
