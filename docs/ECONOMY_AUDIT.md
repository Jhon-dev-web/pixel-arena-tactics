...
Data: 19/09/2026; **atualizado em 20/09/2026 após a integração de Gear Instances** (ver seção 21). Análise do código real e execução do motor atual. Resultado: **Pacote A implementado como primeira etapa (agora portado para o modelo de instâncias individuais); sustentabilidade P2P ainda não demonstrada**. Sem deploy ou blockchain.

> **Como ler este documento depois de Gear Instances.** Os números de emissão, demanda de carreira e simulação supply − sink (seções 3 a 7, 10 e 13 a 17) foram calculados **antes** de Gear Instances, mas continuam matematicamente aplicáveis: dependem do motor da Caça, das receitas e do custo por operação, não da forma como o estado do equipamento é guardado. O que muda com instâncias está na seção 21. Onde o texto original descreve estado compartilhado por tipo de item (`itemId`), ele foi corrigido ou marcado como histórico.
...
| Refine +0..+8 | Gold, ferro/aço/escamas/obsidiana refinada, Dust, shards | Finito **por peça** (cada GearInstance tem o seu nível); o total depende de quantas peças o jogador escolhe melhorar. Antes de Gear Instances era por tipo de item |
...
| Reforge | 1 shard + ouro escalonado pelo reforgeCount **da peça** + Dust + Refino (+ Nobre a partir do T3) | Recorrente enquanto houver busca; consome materiais de caça desde a etapa A (antes: só shard + Gold, zero materiais) |
...
| 10 | 4 | 4 | 4,50 | 4,50 / 1,00 / 0 |
| 20 | 9 | 9 | 9,00 | 9,00 / 2,00 / 0 |
...
Instâncias (**estado atual**): weapon e armor são GearInstances individuais (`gearInstances`, `schemaVersion` 2, commit 15dbf52). Rarity, substats, upgrade, sockets, durabilidade e reforgeCount pertencem a cada peça; craftar outra cópia cria uma peça nova e independente e a reforja afeta somente a peça escolhida. *(Histórico: na primeira versão deste relatório o estado era por `itemId`, o craft de outra cópia sobrescrevia os atributos e a reforja afetava todas as cópias; isso deixou de ser verdade.)* Ainda não há marketplace, servidor nem IDs gerados por servidor: o pré-requisito de instâncias para candidato pendente, lock persistente, venda individual ou Masterwork por peça agora está cumprido no cliente, mas essas funções continuam apenas propostas.
...
*(Seção histórica: descreve a primeira implementação do Pacote A, sobre o modelo por `itemId`. Os testes da portagem para instâncias estão na seção 21.)*
...
*(Primeira implementação, histórica; a portagem atual está listada na seção 21.)* Código alterado: src/App.tsx; src/components/InventoryModal.tsx; src/game/salvage.ts; src/game/tunables.ts; src/locales/en.json; src/locales/pt.json. Novo código: src/game/reforge.ts. Novo relatório: docs/ECONOMY_AUDIT.md. Harness econômico ficou fora do repo no caminho da seção4.
...
## 21. Atualização pós Gear Instances: Pacote A portado para instâncias

**Escopo desta atualização:** a Reforja econômica e o salvage sem mínimo foram portados manualmente para o modelo de GearInstance (branch `feat/economy-sinks`, base develop @ 59a09a5). Não foram alterados Modelo C, drops, CP, combate, Caça, Battle Pass, migração v2 nem legacy-arena.

### 21.1 O que mudou em relação à primeira implementação

| Ponto | Primeira implementação (pré-instâncias) | Agora |
|---|---|---|
| Alvo da reforja | `itemId` (tipo de item); afetava todas as cópias | `instanceId`: só a GearInstance escolhida |
| Contador de reforjas | `save.reforgeCount[itemId]` | `instance.reforgeCount` |
| Rarity / substats | `itemRarity[itemId]`, `itemSubstats[itemId]` | `instance.rarity`, `instance.substats` |
| Custo em Gold | escalonado pelo contador do tipo | escalonado pelo `reforgeCount` **da peça** (10, 25, 50, 90, 150, 240, 360, 500 e 500 dali em diante) |
| Aviso na UI | "afeta todas as cópias deste tipo" | removido; o texto diz que só a peça selecionada é sorteada de novo e que rarity, +N, sockets e durabilidade não mudam |
| Salvage | `floor(0,45 × q)` sem mínimo | idêntico, mas sobre uma instância: só a peça escolhida sai, a equipada é recusada, as gems socketadas voltam ao estoque, o bônus usa a rarity da própria peça |
| Tunables | `T.economySinks` | `T.economySinks` (mesmos valores aprovados) |

### 21.2 Custos portados (T.economySinks, sem recalibração)

| Tier | Pó de Refino | Refino | Nobre | Shard | Gold |
|---|---:|---:|---:|---:|---|
| T1 – T2 | 2 | 6 | – | 1 | 10 → 500 pelo reforgeCount da peça |
| T3 – T6 | 6 | 24 | 2 | 1 | 10 → 500 pelo reforgeCount da peça |

Arma: Refino = Garra Demoníaca, Nobre = Núcleo Demoníaco. Armadura: Refino = Sangue Concentrado, Nobre = Cristal Corrompido. Nenhum material substitui outro; o Pó continua sendo Pó de Refino. Só há reforja em peças de raridade com substats (rare, epic, legendary).

### 21.3 O que instâncias mudam para a economia

- **Consumo por operação:** inalterado (mesmo custo por tentativa). Portanto as tabelas das seções 15 e 16 continuam válidas para o mesmo comportamento de uso.
- **Recraft por raridade:** agora cria uma peça nova e independente sem destruir a antiga (antes sobrescrevia). Cada peça extra abre outra linha de reforja com contador de Gold próprio, o que aumenta a demanda potencial por jogador que colecione peças, mas também prende Gold e materiais de craft. Esse efeito não foi medido.
- **Refine +0..+8:** passa a ser finito por peça, não por tipo; o consumo total depende de quantas peças o jogador decide melhorar.
- **Capacidade:** o limite de instâncias (`T.gear.maxInstances`, default técnico de 60, não balanceado) é separado dos 20 slots da mochila e só bloqueia a criação de novas peças.

### 21.4 Estimativa de reforjas por dia que a emissão sustenta (confirmação, sem alterar custos)

Cálculo feito só para regressão contra a auditoria anterior: emissão medida no motor (Profundo N10, sem BP, valores por hora da zona da família) contra o custo T3–T6 de **6 Pó (= 60 comuns em conversão 10:1) + 24 Refino + 2 Nobres**. As linhas usam os perfis MID, LATE e END (média de END-A e END-B). Não é previsão de uso: mostra o teto da emissão.

| Perfil | Família (zona) | Comum/h | Refino/h | Nobre/h | Reforjas/h limitadas pela caça | 4 h/dia | 8 h/dia numa família |
|---|---|---:|---:|---:|---:|---:|---:|
| MID | arma (glade) | 50,2 | 51,0 | 7,9 | 0,84 (comuns/Pó) | 3,3 | 6,7 |
| MID | armadura (marsh) | 46,7 | 46,8 | 9,8 | 0,78 (comuns/Pó) | 3,1 | 6,2 |
| LATE | arma (rift) | 47,3 | 49,1 | 16,2 | 0,79 (comuns/Pó) | 3,2 | 6,3 |
| LATE | armadura (abyss) | 42,8 | 47,5 | 15,5 | 0,71 (comuns/Pó) | 2,9 | 5,7 |
| END | arma (rift) | 62,4 | 66,5 | 20,3 | 1,04 (comuns/Pó) | 4,2 | 8,3 |
| END | armadura (abyss) | 61,5 | 68,3 | 21,8 | 1,03 (comuns/Pó) | 4,1 | 8,2 |

Leitura:
- Dividindo as 8 h de caça do dia em 4 h por família, a emissão sustenta cerca de **6,4 (MID), 6,1 (LATE) e 8,3 (END) reforjas por dia** somando arma e armadura.
- O limite é o Pó vindo dos comuns da própria caça. Se o Pó vier de cobre ou erva (Mineração/Jardim), o teto passa a ser o Refino: cerca de 2,0 a 2,9 reforjas por hora de caça, ou 8 a 11 por 4 h. O Nobre nunca é o limite (3,9 a 10,9 reforjas por hora).
- O Refino (24 por reforja) é o que mais pesa quando os comuns não limitam. As tabelas da seção 15 já mostravam que a hipótese do Pacote A (1,65 operações por dia por jogador) absorve só 9,99% (garra) e 12,98% (sangue) do Refino em 8 h; esta regressão é consistente com isso.
- Custos e adesão **não** foram ajustados com base nessa conta.

### 21.5 Testes da portagem

Harness fora do repositório (75 verificações de reforja e salvage, mais os 228 de Gear Instances repetidos):
- Reforja: custo exato por tier T1 a T6 e por família; identidade dos materiais; Gold escalonado pelo contador da peça; A (epic, contador 4) e B (rare, contador 0) independentes; faltas isoladas de Gold, Pó, Refino, Nobre e shard (save idêntico, RNG não chamado, contador inalterado); chamadas sequenciais; save/reload; preservação de rarity, upgrade, sockets e durabilidade; valores vindos de `T.economySinks`.
- Salvage: quantidades de receita 1, 2, 3, 5, 10 e 20 devolvem 0, 0, 1, 2, 4 e 9; equipada recusada; cópia não equipada funciona; gems devolvidas; bônus por raridade preservado; outras instâncias intactas.
- Regressão de Gear Instances: cópias independentes, upgrade, socket, durabilidade, CP e Caça idênticos ao motor anterior, save/reload.

### 21.6 Arquivos da portagem

Novo: `src/game/reforge.ts`. Alterados: `src/App.tsx`, `src/components/InventoryModal.tsx`, `src/App.css`, `src/game/salvage.ts`, `src/game/tunables.ts`, `src/game/gearInstances.ts` (removida a reforja simples, agora em `reforge.ts`), `src/game/gear.ts` (só um comentário), `src/locales/pt.json`, `src/locales/en.json`, `docs/ECONOMY_AUDIT.md`.
